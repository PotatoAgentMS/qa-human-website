...
export const KEYS = {
  ...
  fixes: "aihqt:fixes",
  incidents: "aihqt:incidents",
  config: "aihqt:config",
};

export const COLLECTIONS = ["entries", "patterns", "fixes", "incidents"];
...
export async function readAll() {
  const r = redis();
  const [entries, patterns, fixes, incidents, config] = await Promise.all([
    r.hgetall(KEYS.entries),
    r.hgetall(KEYS.patterns),
    r.hgetall(KEYS.fixes),
    r.hgetall(KEYS.incidents),
    r.get(KEYS.config),
  ]);
  return {
